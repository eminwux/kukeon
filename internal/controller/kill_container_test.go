...
func TestKillContainer_SuccessfulKill(t *testing.T) {
	tests := []struct {
		name          string
		...
		wantResult    func(t *testing.T, result controller.KillContainerResult)
		wantErr       bool
	}{
		{
			...
			setupRunner: func(f *fakeRunner) {
				...
				f.KillContainerFn = func(_ intmodel.Cell, containerID string) error {
				...
				f.UpdateCellMetadataFn = func(cell intmodel.Cell) error {
					...
					if len(cell.Spec.Containers) != 2 {
						return errors.New("expected container to remain in cell after kill")
					}
					return nil
				}
			},
		...
		{
			name:          "successful kill with labels preserved",
			containerName: "test-container",
			realmName:     "test-realm",
			spaceName:     "test-space",
			stackName:     "test-stack",
			...
			setupRunner: func(f *fakeRunner) {
				...
				f.KillContainerFn = func(_ intmodel.Cell, _ string) error {
					return nil
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := ctrl.KillContainer(container)
...
func TestKillContainer_ValidationErrors(t *testing.T) {
	tests := []struct {
		name          string
		containerName string
		containerID   string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := ctrl.KillContainer(container)
...
func TestKillContainer_CellNotFound(t *testing.T) {
	tests := []struct {
		name          string
		containerName string
		realmName     string
...
func TestKillContainer_ContainerNotFound(t *testing.T) {
	tests := []struct {
		name          string
		containerName string
		realmName     string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result.Killed {
				t.Error("expected Killed to be false when container not found")
			}
...
func TestKillContainer_RunnerErrors(t *testing.T) {
	tests := []struct {
		name          string
		containerName string
		realmName     string
		...
		{
			name:          "KillContainer runner error is wrapped",
			containerName: "test-container",
			realmName:     "test-realm",
			spaceName:     "test-space",
			stackName:     "test-stack",
			...
			wantErr:     true,
			errContains: "failed to kill container test-container",
		},
		{
...
func TestKillContainer_ContainerLookupByID(t *testing.T) {
	tests := []struct {
		name          string
		containerName string
		containerID   string
...
func TestKillContainer_NameTrimming(t *testing.T) {
	tests := []struct {
		name          string
		containerName string
		realmName     string
