	...
	if cDoc != nil {
		...
		if len(doc.Spec.Containers) > 0 {
			...
			r.logger.DebugContext(
				...
				"cell", cellDocEnsured.Metadata.Name,
				"existingContainerCount", len(cellDocEnsured.Spec.Containers),
				"newContainerCount", len(doc.Spec.Containers),
			)
			...
			for _, container := range doc.Spec.Containers {
				...
				if !existingContainerIDs[container.ID] {
					...
					r.logger.DebugContext(
						...
						"cell", cellDocEnsured.Metadata.Name,
						"containerID", container.ID,
						"totalContainers", len(cellDocEnsured.Spec.Containers),
					)
				}
			}
		}
		...
		r.logger.DebugContext(
			...
			"cell", cellDocEnsured.Metadata.Name,
			"containerCount", len(cellDocEnsured.Spec.Containers),
		)
		...
		if err = r.UpdateCellMetadata(cellDocEnsured); err != nil {
