...
- **Blueprint and Config names for `kuke run -b` / `kuke run -c`** — `kuke run -b <TAB>` / `kuke run -c <TAB>` list the daemon-stored CellBlueprints / CellConfigs visible in the scope named by `--realm`/`--space`/`--stack`. These replace the legacy `-p`/`--profile` filename completion that was removed alongside the `CellProfile` kind in [#626](https://github.com/eminwux/kukeon/issues/626).
