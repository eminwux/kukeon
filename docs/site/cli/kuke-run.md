...
The source Config is never mutated — `kuke run` is a read-and-materialize verb (with the exception of `--clone`, which writes a _new_ Config alongside but never edits the source). To update the source Config's spec, use `kuke apply -c <config>` (which reconciles the at-most-one stable-named cell) or edit the Config manifest and re-apply.

#### Identity-flag mutex matrix

| `--new`                  | `--clone`                | `--reuse` (#835 planned) | `--name`               | `--rm`                    |
| ------------------------ | ------------------------ | ------------------------ | ---------------------- | ------------------------- |
...
| **mutex**                | **mutex**                | —                        | mutex (#835)           | mutex (#835)              |
