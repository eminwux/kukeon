...
```
kuke run (<cell> | -f <file> | --from-blueprint <bp> | --from-config <cfg> | --clone <cell>) [flags]
```
...
| `--file`, `-f`           | _(one source)_                                    | YAML to read (path or `-` for stdin); mutually exclusive with the `<cell>` positional and `--from-blueprint`/`--from-config`/`--clone`                                                                                                                                                                                                                                                                                                                                                     |
...
| `--name`                 | _(generated `<prefix>-<6hex>`)_                   | Name the cell materialised by `--from-blueprint`/`--from-config`/`--clone`. Rejected with the `<cell>` positional (the positional IS the cell name) and with `-f` (where `metadata.name` is authoritative). A `--name X` collision against a live cell exits non-zero with a `kuke run X` pointer for the start+attach-existing path                                                                                                                                                       |
