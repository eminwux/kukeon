...
| -------------------- | -------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `state`              | `Pending`, `Ready`, `Stopped`, `Exited`, `Error`, `Failed`, `Unknown` | Lifecycle state. `Stopped` = operator `kuke stop`/`kill`; `Exited` = all workloads exited 0 (clean self-exit); `Error` = a workload exited non-zero (crash); `Failed` = a kukeon bring-up fault. |
