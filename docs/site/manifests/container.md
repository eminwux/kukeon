...
| `privileged`      | bool                       | no       | Run privileged: full capabilities, writable sysfs/cgroupfs, seccomp/apparmor unconfined, **plus** all host device nodes exposed under `/dev` and an allow-all device cgroup (matches `ctr run --privileged` / `docker run --privileged`), so the container can `open(2)` host devices such as `/dev/kvm`. Host devices are snapshotted into the OCI spec at **create** time — devices that appear on the host afterwards require recreating the container to pick up. |
