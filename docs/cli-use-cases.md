...
sudo kuke run -b <blueprint> --name custom                  # pin cell name "custom" from the Blueprint (create-or-fail on divergence)
...
sudo kuke run <config> --clone                              # fork the Config into a persistent clone <config>-<N>
sudo kuke run <config> --clone --name Y                     # named create-or-fail clone Config
...
- `kuke run -b <blueprint>` (post-#750) materializes a daemon-stored CellBlueprint from the scope named by `--realm`/`--space`/`--stack`, substituting `--param`/`--param-file` scalar overrides into `spec.parameters[]`. Without `--name`, each invocation produces a fresh `<prefix>-<6hex>` cell (24 bits of entropy makes in-realm collisions statistically negligible), so the divergent-spec check does **not** apply — the operator opts into a new cell on every call. With `--name X`, the cell name is pinned to `X` and the materialization walks a name-keyed state machine: attach on a matching live cell, and on divergence default to **warn-and-attach** (post-#986) — a one-line `notice: cell "<name>" is OutOfSync with CellBlueprint "<bp>" (diverging: ...); attaching to current live state — -b cells have no in-place reconcile; delete it with \`kuke delete cell <name>\` and re-run (or promote to a CellConfig for \`kuke restart\` reconcile workflows)`is written to stderr and the operator is dropped into the live cell.`--require-synced` opts into the pre-#986 refusal (same delete-then-rerun pointer, exit non-zero) for CI/scripted callers.
- `kuke run <config>` (the daemon-stored CellConfig positional, post-#825) is **stable-named and idempotent**: the cell name is the Config's `metadata.name`, and re-invocation attaches to the at-most-one live cell the Config owns. A divergent live cell defaults to **warn-and-attach** (post-#986): a one-line `notice: cell "<name>" is OutOfSync with CellConfig "<config>" (diverging: ...); attaching to current live state — run \`kuke restart <name>\` to reconcile (stops, updates, starts)`is written to stderr and the operator is dropped into the live cell.`--require-synced`opts into the pre-#986 refusal (same pointer, exit non-zero) for CI/scripted callers.`--param`/`--param-file` are rejected (the Config carries its own values — edit it instead).
- `kuke run <config> --new` (post-#833) opts out of the stable-name identity: each invocation spawns a fresh cell named `<config-name>-<6hex>` (hex matches `-b`'s naming; 24 bits of entropy makes in-realm collisions statistically negligible). The `kukeon.io/config=<name>` lineage label is preserved so `kuke get cells -l kukeon.io/config=<name>` enumerates every spawn — stable-name, pinned-name, and hex-suffix cells alike.
- `kuke run <config> --name X` (post-#833) is the third identity shape: it pins the materialized cell name to `X` and walks the same idempotent state machine as the bare `<config>` form (attach on collision, refuse on divergence with the same `kuke restart <name>` pointer). `kuke run <config> --new --name X` is the create-or-fail companion: it pins the name to `X` and **fails** if a cell `X` already exists in the realm (error: `cell "X" already exists; --new --name requires the name to be free (use --name "X" alone for attach-if-exists semantics)`). Each form is rejected outside the `<config>` path: `--new` with `-f`/`-b` exits non-zero with `--new is only valid with the <config> positional`.
- `kuke run <config> --clone` (post-#839) is a distinct operation from `--new`: instead of materializing an ephemeral cell, it **forks** the Config into a brand-new persistent CellConfig named `<config>-<N>` and starts a cell from the clone. `N` is the lowest unused integer ≥ 0 among clones of the source in the target realm (gap-fill counter); allocation is atomic — two concurrent `--clone`s against the same source pick distinct `N`'s. The clone carries `metadata.annotations.kukeon.io/source-config: <source>` as the lineage marker and its `spec` is a deep copy of the source's (independently editable via `kuke apply -f`). The cell from the clone follows the stable-name path (`kuke run <config>-N` later resolves the clone and attaches idempotently). `kuke run <config> --clone --name X` is the explicit-name create-or-fail form: it creates clone CellConfig `X` and **fails** with `cellconfig "X" already exists; --clone --name requires the name to be free` on collision (no counter fallback). `--clone` is mutually exclusive with `--new` (different operation), `--reuse` (different operation), and `--rm` (orphans the clone Config). Rejected with `-f`/`-b` (`--clone is only valid with the <config> positional`). Driver use cases: interactive multi-instance (`kukeon-dev-0`, `kukeon-dev-1`, …) and cron pool seeding for `--reuse`.
- `kuke run <config> --reuse` (post-#835) draws from the clone pool seeded by `--clone`: it picks the lowest-counter clone of `<config>` whose cell is in the **Stopped** state and starts that cell in-place via `StartCell` — the daemon creates a new task in the existing containerd snapshot, so the **overlay filesystem persists** across the stop/start transition (project repo clone, `.claude.json`, any per-cell state). `tty.onInit` reruns against the preserved filesystem. On an empty pool — first tick, post-reboot, or every clone already Running — `--reuse` falls through to `--clone`'s code path and allocates a new gap-fill clone (`<source>-<N>`), so the operator never sees a "pool empty" error. Concurrent `--reuse` invocations against the same source pick distinct cells via the daemon's `StartCell` state guard: a `StartCell` against an already-Ready cell returns `must first be stopped`, and the loser transparently advances to the next pool candidate. Running clones are invisible to the pool query (an all-Running pool falls back to fork); cells in Pending / Failed / Unknown sub-states are excluded from the pick set. **Never deletes the picked clone's cell** — the overlay must survive. `--reuse` is mutually exclusive with `--new`, `--clone`, `--name` (can't dictate which slot to pick by name — use `kuke run <clone-name>` for a specific clone), and `--rm` (orphans the clone Config: next tick would see the clone Config in the pool but skip it for the now-missing cell, forcing a fork). Rejected with `-f`/`-b` (`--reuse is only valid with the <config> positional`). Driver use case: cron-driven skill execution where the project repo clone happens once per pool member rather than once per tick (typical line: `kuke run kukeon-dev-pick-issue --reuse --env LABEL=bug -d`).
- `kuke run --env KEY=VALUE` (post-#834) is the **per-invocation runtime knob** for injecting extra env into the cell's attachable container at start time. Repeatable; entries land in the OCI process env of the container picked by the same precedence as `kuke attach` (`tty.default` > first `attachable: true` non-root), in addition to `spec.cell.containers[<attachable>].env`. Empty value (`KEY=`) is allowed; missing `=` is rejected (`--env requires KEY=VALUE, got "..."`); the same KEY supplied twice with different values is rejected (`--env LABEL specified twice with different values ("bug", "enh"); pick one`) rather than silently last-wins. A KEY that collides with an entry in the attachable container's spec env **overrides** the spec value. Valid with all source paths (`<config>`/`-f`/`-b`) and all identity flags (`--new`/`--clone`/`--reuse`); rejected at parse time for malformed input. The injected entries do **not** persist into the cell metadata — the divergent-spec check on a subsequent `kuke run <config>` without `--env` does not trip on the prior injection; the cell's stored spec env is unchanged across restarts. With `--reuse`, each invocation re-injects against the restarted cell; `tty.onInit` scripts run by the cell's attachable container see the injected env on every tick. Distinct from `--param`: `--param` is render-time scalar substitution into the spec (`-b` only); `--env` is start-time runtime injection (every source path). Driver use case: cron-driven skill execution (`kuke run kukeon-dev-pick-issue --reuse --env LABEL=bug -d`) where each tick's CLI args carry the per-tick state without rewriting the Config.
...
- Reconcile fires on any `StartCell` against an OutOfSync + `kukeon.io/config=<name>` cell — `kuke restart`'s start step, `kuke start` directly, and `kuke run <config>` on an existing Stopped cell all trigger the reapply. `kuke stop` + `kuke start` therefore produces the same end state as `kuke restart` (#983).
...
- Running clones of the source Config (`kuke run <config> --clone` or `--reuse` siblings, identified by `metadata.annotations.kukeon.io/source-config: <source>`) are unaffected by this command — each clone is its own cell and needs its own `kuke restart <clone-name>` (or stop+start) to reconcile. The reconcile is per-cell, not per-Config.
...
- `kuke run -f spec.yaml` against an existing cell with a diverging on-disk spec defaults to **warn-and-attach** (post-#986): a one-line `notice: cell "<name>" is OutOfSync with on-disk spec (diverging: ...); attaching to current live state — use \`kuke apply -f\` to update`is written to stderr and the operator is dropped into the live cell. The CLI does **not** mutate the cell on either path. With`--require-synced`(the opt-in strict mode for CI/scripted callers) the same divergence exits non-zero with`live cell "<name>" spec differs from on-disk spec (<fields>) — refusing to attach (--require-synced); use \`kuke apply -f\` to update`. The cell that drove the divergence detection is named explicitly in both shapes so automation can branch.
- `kuke run <config>` against an existing cell whose spec diverges from the current CellConfig (someone edited the Config — or the underlying Blueprint, on a Config materialized from one — after the cell was last materialized) defaults to **warn-and-attach** (post-#986): a one-line `notice: cell "<name>" is OutOfSync with CellConfig "<config>" (diverging: ...); attaching to current live state — run \`kuke restart <name>\` to reconcile (stops, updates, starts)`is written to stderr and the operator is dropped into the live cell.`run`itself never mutates the cell; destructive updates route through`kuke restart <name>`, which picks up the Config divergence and reconciles implicitly. With `--require-synced`the same divergence exits non-zero with`live cell "<name>" spec differs from CellConfig "<config>" (<fields>) — refusing to attach (--require-synced); run \`kuke restart <name>\` to reconcile (stops, updates, starts)`. The same warn-and-attach default and `--require-synced`opt-in apply on the`<config> --name X` path.
- `kuke run -b <blueprint> --name <cell>` against an existing cell `<cell>` whose spec diverges from the current Blueprint materialization defaults to **warn-and-attach** (post-#986): a one-line `notice: cell "<cell>" is OutOfSync with CellBlueprint "<blueprint>" (diverging: ...); attaching to current live state — -b cells have no in-place reconcile; delete it with \`kuke delete cell <cell>\` and re-run (or promote to a CellConfig for \`kuke restart\` reconcile workflows)`is written to stderr and the operator is dropped into the live cell. With`--require-synced`the same divergence exits non-zero with`live cell "<cell>" spec differs from CellBlueprint "<blueprint>" (<fields>) — refusing to attach (--require-synced); -b cells have no in-place reconcile; delete it with \`kuke delete cell <cell>\` and re-run (or promote to a CellConfig for \`kuke restart\` reconcile workflows)`. Blueprint-lineage cells have no implicit reconcile — the operator either deletes and re-runs or promotes the Blueprint to a CellConfig if recurring reconcile is needed.
- The generated-name source paths — `kuke run -b <blueprint>` without `--name` and `kuke run <config> --new` — **skip the divergent-spec check entirely**. Each invocation produces a fresh `<prefix>-<6hex>` cell name (24 bits of entropy makes in-realm collisions statistically negligible), so there is no existing cell whose spec could diverge. The `kukeon.io/config=<name>` lineage label on `<config> --new` spawns still ties them back to the source Config for `kuke get cells -l kukeon.io/config=<name>` enumeration.
- `kuke run <config> --reuse` (post-#835) is **mutex with `--new`, `--clone`, `--name`, and `--rm`**. The first three are different operations from the pool pick (ephemeral cell, persistent fork, name-pinned cell); the `--rm` rejection guards the pool itself — removing the cell on exit would leave a clone Config whose cell is gone, and the next `--reuse` would see the clone in the pool but skip it (cell-less), forcing a fork. The pool would fill with cell-less clone-Config carcasses. Each conflict surfaces cobra's standard mutex error (`if any flags in the group are set none of the others can be`) at parse time. The `--reuse` flag is also rejected on `-f`/`-b` with `--reuse is only valid with the <config> positional` (only the daemon-stored CellConfig path has a clone pool to draw from). On the `<config>` path itself, `--reuse` is non-destructive: it never deletes a clone's cell (overlay preservation is the headline win), never stops a Running cell (Running clones are invisible to the pool query), and on an empty pool falls back to `--clone`'s fork path rather than failing with "pool empty".
