...
- Unlike realms/spaces/stacks, **cells require a source**: `kuke create cell` takes exactly one of `--image`/`--from-blueprint`/`--from-config`/`--clone` (the bare scaffold form is retired). The `--image <ref>` source synthesizes a single attachable container from a bare image ref and persists it stopped (the create-side mirror of `kuke run --image`; `--command` overrides its entrypoint, default `/bin/sh`); it is mutually exclusive with the from-* sources and rejects `--param`/`--param-file`/`--env`. Omitting all four exits non-zero with `kuke create cell requires --image, --from-blueprint, --from-config, or --clone (use 'kuke apply -f <file>' for a full manifest)`.
...
sudo kuke run --image docker.io/library/alpine:3                         # synthesize+create+start+attach a single-container cell from an image ref
sudo kuke run --image docker.io/library/alpine:3 --name my-first         # same, pinned name
sudo kuke run --image docker.io/library/alpine:3 --command /bin/bash     # same, override the synthesized container's entrypoint
```
...
- **Exactly one source is required:** the `<cell>` positional, `-f/--file`, `--image`, or one of `--from-blueprint`/`--from-config`/`--clone`. They are mutually exclusive at parse time; missing all of them exits non-zero with `a source is required: <cell> (start an existing cell), -f/--file, --image, or --from-blueprint/--from-config/--clone (create+start+attach a new one)`.
- `kuke run <cell>` is the start+attach-existing path. The cell must already exist; a missing name exits non-zero with a pointer at `kuke create cell` / `kuke run --from-...`. The transition follows the live state: a Stopped (operator stop/kill), Exited (clean self-exit, #1267), Error (workload crash) or Failed (kukeon bring-up fault) cell is started then attached — all four terminal/degraded states are operator-restartable without a delete (#1268); a Ready cell is attached as a no-op; a genuinely-unrecoverable / mid-transition cell (`Pending`, `Unknown`) is refused with a `kuke delete cell <name>` pointer.
- `kuke run -f` is **create-or-attach keyed by `metadata.name`**. Against a cell whose on-disk spec matches the file, the runtime state selects the transition:
  ...
  - **Stopped / Exited / Error / Failed** → `StartCell`, then attach. All four terminal/degraded states are operator-restartable without a delete (#1268): Stopped (operator stop/kill) and Exited (clean self-exit, #1267) re-run from their intact container records (no re-create); Error (the #1267 workload-crash terminal, a non-zero workload exit whose sticky root container is still live) and Failed (kukeon's own bring-up fault) are routed by the daemon's `StartCell` through a recreate-style recovery (stop → recreate containers → start, including the leftover root) so a crashed or half-created cell recovers without delete-then-recreate (#1274). The live start+attach converges once the CNI duplicate-allocation fix (#630) lands.
  - **Pending / Unknown** → refused with `cell "<name>" exists in <state> state; delete it with \`kuke delete cell <name>\` before re-running`. Exit code non-zero. `run` does not reconcile a genuinely-unrecoverable / mid-transition cell in place.
...
- `kuke run --image <ref>` (epic:first-run #1244) is the **imperative single-image** create source — the quick-start path. It synthesizes a minimal one-container `CellDoc` in-process (the container runs `<ref>`, `attachable: true`, entrypoint `/bin/sh` overridable via `--command`; the runner synthesizes the root container at create time), then create+start+attaches it exactly like the fused paths. Unlike `--from-*`, the synthesis is in-process (`cell.SynthesizeFromImage`) rather than resolved from a daemon-stored binding, so no Blueprint/Config lookup occurs. The cell name is `--name X` if given, else a generated `<prefix>-<6hex>` whose prefix is derived from the image's short name (`docker.io/library/alpine:3` → `alpine-<6hex>`). A `--name X` collision against a live cell exits non-zero with the same `kuke run <cell>` / `kuke delete cell <cell>` pointer the fused paths emit. The `<cell>` positional is rejected with `--image` (it names an existing cell) — the error points the operator at `--name`.
- `--name` is **only** valid with `--image`/`--from-blueprint`/`--from-config`/`--clone`. Rejected with the `<cell>` positional (the positional IS the cell name) and with `-f` (where `metadata.name` is authoritative). `--command` is **only** valid with `--image`. `--param`/`--param-file` are **only** valid with `--from-blueprint`; rejected with `--from-config` (the Config carries its own `spec.values` — edit the Config instead), with `--image`, and with the `<cell>` positional / `-f`.
...
- `--rm` cleanup fires only on a **clean** workload exit (the cell settles `Exited`, #1267 — the reconciler-derived clean self-exit terminal; never `Stopped`, which is reserved for operator stop/kill). A workload that exits non-zero / crashes settles the cell `Error` (#1206 introduced the exit-code split; #1267 renamed the workload-crash terminal from `Failed` to `Error` and reserved `Failed` for kukeon's own bring-up faults). `Error` is excluded from auto-delete — the crashed cell is preserved with its exit code/signal in `reason`/`message` so the failure can be inspected, and is cleaned up with an explicit `kuke delete cell`.
...
- Only a genuinely-unrecoverable / mid-transition cell (`Pending` / `Unknown`) refuses with a `kuke delete cell <name>` pointer; restart does not reconcile those in place. The terminal/degraded states `Stopped` (operator stop/kill), `Exited` (clean self-exit, #1267), `Error` (workload crash) and `Failed` (kukeon bring-up fault) all take the `restartStopped` path — `StartCell` with no stop-first, so the daemon observes the persisted state and recovers `Failed` via a recreate-style path (stop → recreate containers → start) while the others re-run from intact records (#1268). A stop-first on a `Failed` cell would flip it to `Stopped` and skip that recovery, so restart never stops a non-Ready cell.
- `kuke start`, `kuke run <cell>`, and `kuke restart` agree on the startable-state set — the decision lives in the daemon's `controller.StartCell`, the single funnel all three reach (#1268). It refuses `Pending` / `Unknown` with the `kuke delete cell <name>` pointer, recovers `Failed` via `RecreateCell`, and starts `Stopped` / `Exited` / `Error` (and a Ready cell whose containerd task is gone) from their existing records. Before #1268, `kuke start` had no state guard and would try to start a `Failed` cell the other two verbs refused; a successful start clears the cell's failure breadcrumb (`Status.Reason` / `Status.Message`) on the Ready transition.
...
The `--no-daemon` flag was removed from the remaining daemon-routed workload commands (`apply`, `create`, `run`, `attach`, `delete`, `kill`, `start`, `stop`, `log`, `refresh`) by #222 — that workload-command removal is the current state. The flag is still accepted on `kuke init`, `kuke uninstall`, `kuke purge`, and every `kuke get <kind>` (the `get` kinds were retained per a user override on the original AC so the in-process escape hatch stays available for every resource lookup, not just `get realm` for the daemon-parity check, retired by #223 once `kuke status` (#202) absorbs it). For the true workload verbs (`apply`, `create *`, `run`, `attach`, `delete *`, `kill *`) the in-process branch is **no longer reachable at all**: #566/#588 routed them through the daemon-only client, which ignores `kukeon/noDaemon`, so `KUKEON_NO_DAEMON=true` and the `--run-path` promotion are inert for them. The promotion stays valid only for the surviving daemon-routed-but-promotable callers — `get *`, `purge *`, `log`, `refresh`, `restart`, `start`, `stop`, `doctor cgroups` — plus the bootstrap commands `init` and `uninstall`. The long-term arc deletes that surviving in-process branch entirely under #566.
...
- In-process variants — `kuke get realms --no-daemon`, `kuke purge realm --cascade --force --no-daemon`, or a surviving promotable caller (`get *`, `purge *`, `log`, `refresh`, `restart`, `start`, `stop`, `doctor cgroups`) run with `KUKEON_NO_DAEMON=true` / an explicit `--run-path` — continue to work for in-process-controller-supported operations (subject to root + a usable `/run/containerd/containerd.sock`). The true workload verbs (`apply`, `create *`, `run`, `attach`, `delete *`, `kill *`) route through the daemon-only client and have no in-process fallback — they always require the daemon.
