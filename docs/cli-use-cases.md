...
- `kuke run <config> --name X` (post-#833) is the third identity shape: it pins the materialized cell name to `X` and walks the same idempotent state machine as the bare `<config>` form (attach on collision, refuse on divergence with the same `kuke restart cell <name>` pointer). `kuke run <config> --new --name X` is the create-or-fail companion: it pins the name to `X` and **fails** if a cell `X` already exists in the realm (error: `cell "X" already exists; --new --name requires the name to be free (use --name "X" alone for attach-if-exists semantics)`). Each form is rejected outside the `<config>` path: `--new` with `-f`/`-b` exits non-zero with `--new is only valid with the <config> positional`.
...
- `kuke run <config>` against an existing cell whose spec diverges from the current CellConfig (someone edited the Config — or the underlying Blueprint, on a Config materialized from one — after the cell was last materialized) defaults to **warn-and-attach** (post-#986): a one-line `notice: cell "<name>" is OutOfSync with CellConfig "<config>" (diverging: ...); attaching to current live state — run \`kuke restart cell <name>\` to reconcile (stops, updates, starts)` is written to stderr and the operator is dropped into the live cell. `run` itself never mutates the cell; destructive updates route through `kuke restart cell <name>`, which picks up the Config divergence and reconciles implicitly. With `--require-synced` the same divergence exits non-zero with `live cell "<name>" spec differs from CellConfig "<config>" (<fields>) — refusing to attach (--require-synced); run \`kuke restart cell <name>\` to reconcile (stops, updates, starts)`. The same warn-and-attach default and `--require-synced` opt-in apply on the `<config> --name X` path.
- `kuke run -b <blueprint> --name <cell>` against an existing cell `<cell>` whose spec diverges from the current Blueprint materialization defaults to **warn-and-attach** (post-#986): a one-line `notice: cell "<cell>" is OutOfSync with CellBlueprint "<blueprint>" (diverging: ...); attaching to current live state — -b cells have no in-place reconcile; delete it with \`kuke delete cell <cell>\` and re-run (or promote to a CellConfig for \`kuke restart cell\` reconcile workflows)` is written to stderr and the operator is dropped into the live cell. With `--require-synced` the same divergence exits non-zero with `live cell "<cell>" spec differs from CellBlueprint "<blueprint>" (<fields>) — refusing to attach (--require-synced); -b cells have no in-place reconcile; delete it with \`kuke delete cell <cell>\` and re-run (or promote to a CellConfig for \`kuke restart cell\` reconcile workflows)`. Blueprint-lineage cells have no implicit reconcile — the operator either deletes and re-runs or promotes the Blueprint to a CellConfig if recurring reconcile is needed.
