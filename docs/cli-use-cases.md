...
- On the reconcile branch, the CLI composes `GetConfig` + `GetBlueprint` + `ApplyDocuments` (at CLI level — no new daemon RPC), then issues an explicit `StopCell` + `StartCell`. The daemon's reconcile path only bounces containers on image / command / args divergence, so the explicit stop + start preserves the `kuke restart` contract that every container in the cell bounces — even on env-only or metadata-only divergence classes.
...
- `kuke run <config>` against an existing cell whose spec diverges from the current CellConfig (someone edited the Config — or the underlying Blueprint, on a Config materialized from one — after the cell was last materialized) exits non-zero with `live cell "<name>" spec differs from CellConfig "<config>" (<fields>) — refusing to attach; use \`kuke restart cell <name>\` to reconcile (stops, updates, starts)`. `run` stays a pure read-and-materialize verb; destructive updates route through `kuke restart cell <name>`, which picks up the Config divergence and reconciles implicitly. The same pointer applies on the `<config> --name X` path.
- `kuke run -b <blueprint> --name <cell>` against an existing cell `<cell>` whose spec diverges from the current Blueprint materialization exits non-zero with `live cell "<cell>" spec differs from CellBlueprint "<blueprint>" (<fields>) — refusing to attach; -b cells have no in-place reconcile; delete it with \`kuke delete cell <cell>\` and re-run (or promote to a CellConfig for \`kuke restart cell\` reconcile workflows)`. Blueprint-lineage cells have no implicit reconcile — the operator either deletes and re-runs or promotes the Blueprint to a CellConfig if recurring reconcile is needed.
