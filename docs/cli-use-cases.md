...
- `kuke run <cell>` is the start+attach-existing path. The cell must already exist; a missing name exits non-zero with a pointer at `kuke create cell` / `kuke run --from-...`. The transition follows the live state: a Stopped (operator stop/kill) or Exited (clean self-exit, #1267) cell is started then attached; a Ready cell is attached as a no-op; an error / partial cell (`Pending`, `Failed`, `Error`, `Unknown`) is refused with a `kuke delete cell <name>` pointer.
- `kuke run -f` is **create-or-attach keyed by `metadata.name`**. Against a cell whose on-disk spec matches the file, the runtime state selects the transition:
  ...
  - **Stopped / Exited** → `StartCell`, then attach (no re-create). Both clean terminals (operator stop/kill, or a clean self-exit, #1267) restart in place. The prior fall-through to create was an unsafe re-entry; the live start+attach converges once the CNI duplicate-allocation fix (#630) lands.
  - **Error / partial** (`Pending`, `Failed`, `Error`, `Unknown`) → refused with `cell "<name>" exists in <state> state; delete it with \`kuke delete cell <name>\` before re-running`. Exit code non-zero. `run` does not reconcile a degraded cell in place. `Error` is the #1267 workload-crash terminal (a non-zero workload exit) — distinct from `Failed`, kukeon's own bring-up fault.
...
- A degraded cell (`Pending` / `Failed` / `Error` / `Unknown`) refuses with a `kuke delete cell <name>` pointer; restart does not reconcile a degraded cell in place. The clean terminals `Stopped` (operator stop/kill) and `Exited` (clean self-exit, #1267) take the `restartStopped` bounce path.
