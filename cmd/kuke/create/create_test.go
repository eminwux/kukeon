...
import (
	...
	"github.com/eminwux/kukeon/cmd/kuke/create"
	"github.com/spf13/cobra"
)

func TestNewCreateCmdMetadata(t *testing.T) {
	tests := []struct {
		name  string
	...
	}{
		{
			...
			check: func(t *testing.T, cmd *cobra.Command) {
				if cmd.Use != "create" {
					t.Fatalf("expected Use to be %q, got %q", "create", cmd.Use)
				}
			},
		},
		{
			...
			check: func(t *testing.T, cmd *cobra.Command) {
expected := "Create Kukeon resources (realm, space, stack, cell, container, config, blueprint, secret)"
...
func TestNewCreateCmdRegistersSubcommands(t *testing.T) {
	tests := []struct {
		name string
	}{
		{name: "realm"},
...
func TestNewCreateCmd_AutocompleteRegistration(t *testing.T) {
	cmd := create.NewCreateCmd()
...
expected := []string{"realm", "space", "stack", "cell", "container", "config", "blueprint", "secret"}
